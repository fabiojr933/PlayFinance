...
const ImpostoEdit = () => {
  ...
  async function handleSalvar(e) {
    ...
    try {
      ...
      if (resposta.status == 201 || resposta.status == 200) {
        ...
        toast.info('imposto alterado com sucesso');
      }
    } catch (error) {
      toast.error(error.response.data.error);
    }
  ...
  return (
    <div className="main-content-container p-4 container-fluid">
      <div >
        <Button onClick={() => { history.push('/dashboard/imposto') }} type="button" className="button button-primary">
          Voltar
        </Button>
      </div><br />
      <h2 style={{ textAlign: "center" }}>
        <Badge bg="secondary">Editar de imposto</Badge>
      </h2>
      <Card>
        <div style={{ marginTop: 20, marginBottom: 20 }}>
          <Form className="col-sm-4" onSubmit={handleSalvar}>
            <Form.Group className="mb-2" >
              ...
              <Form.Control type="text" placeholder="Digite o nome da imposto" value={nome} onChange={(e) => { setNome(e.target.value) }} />
            </Form.Group>
